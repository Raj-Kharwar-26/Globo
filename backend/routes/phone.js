...
router.post('/generate', auth, async (req, res) => {
  ...
  try {
    ...
    if (numberExists) {
      ...
      return res.status(400).json({ message: 'Number already exists. Please try again.' });
...
router.get('/user/:number', auth, async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      return res.status(404).json({ message: 'No active user found with this number' });
...
router.post('/rent/:numberId', auth, async (req, res) => {
  ...
  try {
    ...
    user.virtualNumbers[numberIndex].expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
