...
export const useWebRTC = ({ socket, userId, onError, user }: UseWebRTCProps) => {
  ...
  useEffect(() => {
    ...
    socket.on("incoming-call", async ({ from, offer, callerNumber, recipientNumber }) => {
      console.log("Incoming call from:", callerNumber, "to:", recipientNumber);
  ...
  const makeCall = async (toUserId: string, recipientNumber: string) => {
    try {
      const activeNumber = user?.virtualNumbers?.find(n => n.active && n.rented)?.number;
      if (!activeNumber) {
        throw new Error("No active virtual number found");
      }

      console.log("\nInitiating call:");
      console.log("=================");
      console.log("From:", activeNumber);
